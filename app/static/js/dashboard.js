window.PlaylistDashboard = () => {  
    ...
    React.useEffect(() => {
        ...
        console.log("Full playlist data:", playlistData);
        console.log("Playlist names:", playlistData.map(p => p.playlist_name));
        ...
        const trace = {
            type: 'scatter3d',
            mode: 'markers+text',
            x: playlistData.map(p => {
                return p.avgPopularity;
            }),
            y: playlistData.map(p => {
                return p.genreCount;
            }),
            z: playlistData.map(p => {
                return p.avgYear;
            }),
            text: playlistData.map(p => {
                console.log('Full playlist object:', p);
            ...
            hovertext: playlistData.map(p => 
                ...
                `Avg Song Popularity: ${Math.round(p.avgPopularity)}<br>` +
                `Number of Genres: ${p.genreCount}<br>` +
                `Avg Song Year: ${p.avgYear.toFixed(1)}`
            )
        };
        ...
        const layout = {
            scene: {
                ...
                aspectratio: { x: 1, y: 1, z: 0.75 }  // Make plot wider
            },
        ...
        // Always recreate plot to ensure text labels are shown
    ...
    React.useEffect(() => {
        ...
        Plotly.newPlot(plotRef.current, traces, layout, config).then(() => {
            ...
            plotRef.current.on('plotly_click', (data) => {
                ...
                // Only respond to clicks on the main data series (index 0)
